...
Cada aplicação foi executada com diferentes quantidades de \textit{threads} (1, 2, 4 e 8), totalizando 10 execuções por configuração para reduzir variações no ambiente. Nos algoritmos que envolvem aleatoriedade, como \texttt{perfo} e \texttt{drop}, foi utilizada uma semente fixa para garantir a reprodutibilidade dos resultados. As técnicas \texttt{perfo}, \texttt{drop} e \texttt{memo} foram testadas com cinco níveis de taxa de exclusão: 0.1, 0.2, 0.3, 0.4 e 0.5.
...
Para avaliar a perda de acurácia das aplicações, foram utilizadas duas métricas distintas: o \gls{smape} e o \gls{mcr}, definidos, respectivamente, em \autoref{eq:smape} e \autoref{eq:mcr}.
...
Os experimentos realizados durante esse trabalho foram sumarizado na \autoref{tab:benchmarks}.
...
\begin{table}[htb]
	...
	\begin{tabular}{|l|l|l|l|}
		...
		\hline
		2MM                & PolyBench~\cite{polybench}                         & Álgebra Linear              & SMAPE                         \\
		\hline
		Correlação         & PolyBench~\cite{polybench}                         & Probabilidade e Estatística & SMAPE                         \\
		\hline
		Jacobi 2D          & PolyBench~\cite{polybench}                         & Solução Numérica            & SMAPE                         \\
		\hline
		K-Means            & Rodinia~\cite{rodinia}                             & Aprendizado de Máquina      & MCR                           \\
		\hline
		Mandelbrot         & Debian Benchmarks Game~\cite{debianBenchmarksGame} & Visualização Computacional  & SMAPE                         \\
		\hline
		PI de Monte Carlo  & Debian Benchmarks Game~\cite{debianBenchmarksGame} & Probabilidade e Estatística & SMAPE                         \\
		\hline
	\end{tabular}
	\fonte{}

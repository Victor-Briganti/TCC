...
Cada aplicação foi executada com 1, 2, 4 e 8 \textit{threads} em 10 execuções por configuração para incluir variações no ambiente, todas compiladas com \textit{flag} de compilação \texttt{-O3} (nível de maior otimização do código gerado pelo compilador). Nos algoritmos que envolvem aleatoriedade, como \texttt{perfo} e \texttt{drop}, foi utilizada uma semente fixa para garantir a reprodutibilidade dos resultados. As técnicas \texttt{perfo}, \texttt{drop} e \texttt{memo} foram testadas com cinco níveis de taxa de exclusão: 0.1, 0.2, 0.3, 0.4 e 0.5. Esses valores foram escolhidos por representarem níveis progressivos de descarte, variando de 10\% a 50\% da computação, permitindo observar de maneira controlada como cada aplicação reage a aumentos graduais de aproximação e identificar o ponto onde o impacto no desempenho ou na qualidade se torna mais significativo.
...
No \autoref{alg:correlation_matrix} essa propriedade de simétrica é explorada, e é também nessa região de código, mais especificamente em seu laço mais externo, que foram adicionados as anotações de paralelização, permitindo que o cálculo dos coeficientes fossem executados concorrentemente.
...
As métricas de qualidade foram definidas conforme a natureza da saída produzida por cada aplicação. Para aplicações com resultados numéricos contínuos, como 2MM, Correlação, Jacobi 2D e PI de Monte Carlo, adotou-se o \gls{smape}, pois essa métrica permite comparar valores aproximados e exatos mesmo quando variam de escala. No caso do K-Means, utilizou-se o \gls{mcr} por ser uma métrica específica para avaliar a precisão da atribuição dos pontos aos \textit{clusters}. Já na aplicação Deriche, cujo resultado é uma imagem filtrada, optou-se pelo \textit{ssim}, que mede a similaridade estrutural entre as imagens.
...
\begin{enumerate}
	...
	\item Escrita do trabalho de conclusão.
	\item Defesa do projeto de Trabalho de Conclusão de Curso (TCC1).
	\item Defesa do projeto de Trabalho de Conclusão de Curso (TCC2).
\end{enumerate}
